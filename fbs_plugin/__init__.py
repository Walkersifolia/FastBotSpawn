...
def save_config(server: ServerInterface):
    with open(config_file, 'w') as f:
        json.dump({'prefix': prefix, 'limit': limit, 'suffix': suffix}, f)

def load_config(server: ServerInterface):
    global prefix, limit ,suffix
...
def clear_suffix(server: ServerInterface):
    global suffix
    suffix = ''
    save_config(server)
    server.logger.info('Suffix cleared')
...
def spawn_bots(source: CommandSource, start: int, end: int):
    ...
    for i in range(start, end+1):
        source.get_server().execute(f'/execute at {player} run player {prefix}{i}{suffix} spawn in survival')

@new_thread(PLUGIN_METADATA['name'])
def drop_items(source: CommandSource, start: int, end: int):
    for i in range(start, end+1):
        source.get_server().execute(f'/player {prefix}{i}{suffix} dropStack all')

@new_thread(PLUGIN_METADATA['name'])
def kill_bots(source: CommandSource, start: int, end: int):
    for i in range(start, end+1):
        source.get_server().execute(f'/player {prefix}{i}{suffix} kill')

def show_help_message(server: ServerInterface, info: Info):
    server.reply(info, '\n----------欢迎使用FastBotSpawn插件----------\n')
    server.reply(info, '§6!!b pre bot_§r 设置前缀，§6bot_§r可替换，推荐和Carpet配置同步，没有就留空')
    server.reply(info, '§6!!b suf _fake§r 设置后缀，§6_fake§r可替换，推荐和Carpet配置同步，没有就留空')
    server.reply(info, '§6!!b limit [number]§r 设置最大生成数量，默认为10')
    server.reply(info, '§6!!b clear§r 清除设置的前缀&后缀')
...
def on_user_info(server: ServerInterface, info: Info):
    global prefix, limit ,suffix
    if info.is_player:
        if info.content.startswith('!!b pre'):
            prefix = info.content.split(' ')[2]
            save_config(server)
            server.reply(info, f'假人前缀已设置为 "{prefix}"')
        elif info.content.startswith('!!b suf'):
            suffix = info.content.split(' ')[2]
            save_config(server)
            server.reply(info, f'假人后缀已设置为 "{suffix}"')
        elif info.content == '!!b clear':
            clear_prefix(server)
            clear_suffix(server)
            server.reply(info, '假人前缀&后缀已清除')
